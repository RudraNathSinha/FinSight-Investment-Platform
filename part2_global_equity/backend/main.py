...
DB_PATH = os.path.join(_BASE, "data", "stocks.db")
FRONTEND = os.path.join(_BASE, "frontend")
# Monorepo root (FinSightApplication) when launched via start_finsight.py
FINSIGHT_ROOT = os.environ.get("FINSIGHT_ROOT") or os.path.abspath(os.path.join(_BASE, ".."))
LANDING_DIR = os.path.join(FINSIGHT_ROOT, "landing")
if not os.path.isdir(LANDING_DIR):
    LANDING_DIR = os.path.join(FRONTEND, "_landing")
PART1_DIR = os.path.join(FINSIGHT_ROOT, "part1_country_ranking")
PART3_URL = os.environ.get("FINSIGHT_PART3_URL", "http://127.0.0.1:5000")
RANKING_DIR = os.path.join(_BASE, "data", "country_ranking")
...
@app.get("/")
def root():
    # Prefer FinSight landing page when monorepo is present
...
@app.get("/sector/{name}")
def spa_fallback(code: str = None, symbol: str = None, name: str = None):
    return FileResponse(os.path.join(FRONTEND, "index.html"))


# FinSight monorepo static mounts (landing + country ranking)
...
if os.path.isdir(PART1_DIR):
    # Prefer full Part 1 tree (includes data/)
    try:
        app.mount("/country-ranking", StaticFiles(directory=PART1_DIR, html=True), name="country-ranking-part1")
    except Exception:
        pass

if os.path.isdir(FRONTEND):
    app.mount("/static", StaticFiles(directory=FRONTEND), name="static")
    ranking_dir = os.path.join(FRONTEND, "country-ranking")
    if os.path.isdir(ranking_dir):
        app.mount("/country-ranking", StaticFiles(directory=ranking_dir, html=True), name="country-ranking")
