...
(function () {
  ...
  document.addEventListener('click', function (ev) {
    const a = ev.target.closest('#explore-listings-btn');
    if (!a) return;
    const iso3 = state.currentCountry;
    if (!iso3) return;
    const base = (window.FINSIGHT_PART2_URL || '../part2_global_equity/').replace(/\/?$/, '/');
    const name = encodeURIComponent(countryName(iso3));
    a.href = base + '?country=' + encodeURIComponent(iso3) + '&country_name=' + name;
  });
    ...
    el.innerHTML = `
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div class="mb-6">
          ...
          <p class="text-sm text-ink-500 mt-1 max-w-2xl">Country ranking only (not sector or stock ranking). ${state.ranks.length} economies with listed stock exchanges, ordered by final score (0–100). Click a country to see which dimensions and indicators explain its position.</p>
        </div>
      ...
      <div class="bg-white border-b border-ink-200">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 sm:gap-6">
              <a id="explore-listings-btn" href="#" target="_blank" rel="noopener"
                 class="inline-flex items-center justify-center px-3 py-2 rounded-md text-xs font-semibold bg-accent-600 text-white hover:bg-accent-700">
                Explore listings in this country →
              </a>
    ...
    try {
      ...
      $('#app-loading').innerHTML = `
        <div class="text-center px-4">
          ...
          <p class="text-xs text-ink-400 mt-3">This is the Country Ranking workspace (not sector or stock ranking). Ensure the data/ folder is deployed next to this page (rank_list.json and related files). On Render, include part1_country_ranking/data in the static publish directory.</p>
        </div>
      `;
    }
  }
